...
unsigned long lastReadTime = 0;  // Store the time of the last read
unsigned long readInterval = 1000; // 500 ms interval
...
void setup()
{
  ...
  cli();
  //TCCR0B = (TCCR0B & 0b11111000) | 0x02; //7.81250[kHz] pin 13,4  62.5[kHz]  dont change this one actually. its too complicated to try and compensate the millis and micros functions.
                  ...
                  
{
        ...
        if (currentTime - lastReadTime >= readInterval) {
            hall1 = ads.readADC_SingleEnded(1);  // Sensor 2 on A1  ---> purple wire goes to Pin 9 on the connector terminal which is connected to the Z hall sensor
            hall2 = ads.readADC_SingleEnded(2);  // Sensor 3 on A2 ---> blue wire goes to Pin 7 on the connector terminal which is connected to the Y hall sensor
            hall3 = ads.readADC_SingleEnded(3);  // Sensor 3 on A3 ---> blue wire goes to Pin 5 on the connector terminal which is connected to the X hall sensor
    ...
    if  (myTransfer.available()){ 
              ...
              uint16_t recSize = 0;
              recSize = myTransfer.rxObj(action,recSize);  
